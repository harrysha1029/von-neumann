...
var $author$project$Main$initialLevel = 0;
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		A2($author$project$Main$Model, $author$project$Main$initialLevel, 0),
...
var $author$project$Levels$set = function (x) {
	return '\\{' + (A2($elm$core$String$join, ',', x) + '\\}');
};
var $author$project$Levels$five = {
	aka: _List_fromArray(
		['\\wp(V_4)']),
	card: '2^{16} = 65536',
	...
	info: _List_fromArray(
		['Woowww that\'s a lot of sets. So many that I\'m not gonna put them all here..', 'But at this stage we can start to see some cool patterns emerging. An obvious one is that \\(V_{n-1} \\in V_n\\), as every set is a subset of itself. A less obvious one is \\(V_{n-1} \\subseteq V_n\\) (why?). Actually something even cooler holds...']),
	name: 'V_5'
};
var $author$project$Levels$four = {
	aka: _List_fromArray(
		['\\wp(V_3)']),
	card: '16',
	contents: _List_fromArray(
...
var $author$project$Levels$n = {
	...
	info: _List_fromArray(
		['We saw that \\(V_6\\) was already much bigger than the universe yet it didn\'t even contain the number 6! So it\'s clear that we can\'t stop at 6 if we want the Von Neumann Universe to host all of mathematics!', 'So we continue to take powersets for each \\(n\\). Note that no \\(V_n\\) will contain all the natural numbers becuase it won\'t contain the number \\(n\\).']),
	name: 'V_n'
};
var $author$project$Levels$omega = {
	aka: _List_Nil,
	card: '\\omega = \\aleph_0',
	contents: _List_fromArray(
		['\\text{Any } n', '\\text{Any } V_{n}', '\\text{Any hereditarily finite set}', '...']),
	info: _List_fromArray(
...
var $author$project$Levels$omega5 = {
	...
	card: '\\text{Really big}',
	contents: _List_fromArray(
		['\\mathbb{Q}', '\\mathbb{Z}', '\\text{All natural numbers, integers, and rationals}', '...']),
	info: _List_fromArray(
...
var $author$project$Levels$omega6 = {
	...
	info: _List_fromArray(
		['Each \\(r \\in \\mathbb{R} \\) is a subset of \\(\\mathbb{Q}\\), so \\(\\mathbb{R}\\) can be found in \\(V_{\\omega+6}\\)']),
	name: 'V_{\\omega+6}'
};
var $author$project$Levels$omegao = {
	aka: _List_Nil,
	card: '\\text{Ridiculous}',
	contents: _List_fromArray(
...
var $author$project$Levels$omegaomega = {
	aka: _List_Nil,
...
var $author$project$Levels$omegas = {
	aka: _List_Nil,
	card: '\\text{Gigantic}',
	contents: _List_fromArray(
...
var $author$project$Levels$six = {
	...
	info: _List_fromArray(
		['In just 6 steps, we have made a set with \\(2^{65536}\\) elements.', 'To put that into perspective, even a super sharp 4k screen has less than \\(2^{30}\\) pixels so it\'s impossible to display each set on the screen at once.', 'Actually, the number of atoms in the universe is less than \\(2^{300}\\), so it\'s safe to say that \\(V_6\\) is significantly bigger than the physical universe.']),
	name: 'V_6'
};
var $author$project$Levels$three = {
	aka: _List_fromArray(
		['\\wp(V_2)']),
	card: '4',
	contents: _List_fromArray(
	...
	info: _List_fromArray(
		['One consequence of the fact that \\(V_n = \\wp(V_{n-1})\\), is that \\(|V_n| = 2^{|V_{n-1}|}\\).']),
	name: 'V_3'
};
var $author$project$Levels$two = {
	aka: _List_fromArray(
		['2', '\\wp(V_1)']),
	card: '2',
	contents: _List_fromArray(
		['0', '1']),
	info: _List_fromArray(
		['Continue by taking the powerset of the previous stage.', 'So far, \\(V_i = i\\) if we think of \\(i\\) as a Von Neumann ordinal, but this trend will soon end.']),
	name: 'V_2'
};
var $author$project$Levels$v = {
	aka: _List_Nil,
	card: '??',
	contents: _List_fromArray(
		['\\text{Everything?!}']),
...
var $author$project$Levels$levels = $elm$core$Array$fromList(
	_List_fromArray(
		[$author$project$Levels$zero, $author$project$Levels$one, $author$project$Levels$two, $author$project$Levels$three, $author$project$Levels$four, $author$project$Levels$five, $author$project$Levels$six, $author$project$Levels$n, $author$project$Levels$omega, $author$project$Levels$omega1, $author$project$Levels$omega2, $author$project$Levels$omega5, $author$project$Levels$omega6, $author$project$Levels$omegao, $author$project$Levels$omegas, $author$project$Levels$omegaomega, $author$project$Levels$v]));
var $author$project$Main$numLevels = $elm$core$Array$length($author$project$Levels$levels);
var $elm$json$Json$Encode$null = _Json_encodeNull;
var $author$project$Main$reloadMath = _Platform_outgoingPort(
	'reloadMath',
	function ($) {
		...
		switch (msg.$) {
			...
			case 'Up':
				return (_Utils_cmp(model.l + 2, $author$project$Main$numLevels) > 0) ? _Utils_Tuple2(model, $elm$core$Platform$Cmd$none) : _Utils_Tuple2(
					_Utils_update(
						model,
						{k: model.k + 1, l: model.l + 1}),
					$author$project$Main$reloadMath(_Utils_Tuple0));
			case 'Down':
				return ((model.l - 1) < 0) ? _Utils_Tuple2(model, $elm$core$Platform$Cmd$none) : _Utils_Tuple2(
					_Utils_update(
						model,
						{k: model.k + 1, l: model.l - 1}),
					$author$project$Main$reloadMath(_Utils_Tuple0));
...
var $author$project$Main$footer = A2(
	...
	$mdgriffith$elm_ui$Element$text('harryshahai@gmail.com'));
...
var $author$project$Main$mySlider = function (model) {
	return A2(
			...
			]),
		{
			...
			max: $author$project$Main$numLevels - 1,
			min: 0,
			onChange: A2($elm$core$Basics$composeR, $elm$core$Basics$round, $author$project$Main$ChangeSlider),
...
var $author$project$Main$render = function (model) {
	var indexed = A2($elm$core$Array$get, model.l, $author$project$Levels$levels);
	if (indexed.$ === 'Nothing') {
				...
				A2(
						...
						[
							...
							$mdgriffith$elm_ui$Element$Background$gradient(
							{
								...
								steps: _List_fromArray(
									[
										A3($mdgriffith$elm_ui$Element$rgb255, 221, 199, 235),
										A3($mdgriffith$elm_ui$Element$rgb255, 211, 169, 205)
									])
							}),
					...
					_List_fromArray(
						[
							...
							$mdgriffith$elm_ui$Element$text('The Von Neumann Universe')),
							A2(
							$mdgriffith$elm_ui$Element$row,
							_List_fromArray(
								[
							...
							_List_fromArray(
								[
									$author$project$Main$mySlider(model),
